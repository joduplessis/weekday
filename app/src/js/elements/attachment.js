...
const AttachmentComponent = props => {
  ...
  return (
    <Container preview={props.preview} fullwidth={props.fullwidth || false}>
      ...
      <ContainerRow>
        <Icon color={getMimeTypeColor(props.mime)}>{getMimeTypeIcon(props.mime)}</Icon>

        <Content>
          <Name>{props.name}</Name>
          {props.size && <Size>{bytesToSize(props.size)}</Size>}
          <Info>
            <Extension>{getMimeTypeDescription(props.mime)}</Extension>

            <Link onClick={() => window.open(props.uri)}>Download</Link>

            {props.onPreviewClick && props.preview && <Link onClick={props.onPreviewClick}>Preview</Link>}
          </Info>
        </Content>
      </ContainerRow>
    </Container>
...
export const Attachment = React.memo(props => <AttachmentComponent {...props} />)
